...
public class TargetConnectionManager {
     ...
     * automatically closed after {@link NetworkModule.MAX_TARGET_TTL}. For long-running operations
     * which may hold the connection open and active for longer than the configured TTL, this method
     * provides a way for the consumer to inform the {@link TargetConnectionManager} and its
     * internal cache that the connection is in fact still active and should not be expired/closed.
     * This will extend the lifetime of the cache entry by another TTL into the future from the time
     * this method is called. This may be done repeatedly as long as the connection is required to
     * remain active.
     *
