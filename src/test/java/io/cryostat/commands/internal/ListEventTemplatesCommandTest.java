...
import io.cryostat.TestException;
import io.cryostat.commands.Command;
import io.cryostat.commands.SerializableCommand;
import io.cryostat.commands.SerializableCommand.ListOutput;
import io.cryostat.commands.SerializableCommand.Output;
...
class ListEventTemplatesCommandTest implements ValidatesTargetId {
    ...
    void executeShouldPrintListOfTemplateNames() throws Exception {
        ...
        inOrder.verify(cw)
                .println(
                        "\t[Cryostat]\tALL:\tEnable all available events in the target JVM, with default option values. This will be very expensive and is intended primarily for testing Cryostat's own capabilities.");
    }
