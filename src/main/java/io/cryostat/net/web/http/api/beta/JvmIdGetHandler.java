...
class JvmIdGetHandler extends AbstractAuthenticatedRequestHandler {
    ...
    @Inject
    JvmIdGetHandler(
            AuthManager auth, CredentialsManager credentialsManager, Logger logger, RecordingMetadataManager recordingMetadataManager) {
    ...
    public void handleAuthenticated(RoutingContext ctx) {
        ...
        try {
            String jvmId = this.recordingMetadataManager.getJvmId(targetId);
            ctx.response().end(jvmId);
        } catch (Exception e) {
            logger.error("Couldn't get the jvmId from {}", targetId);
            throw new HttpException(699);
        }
    }
}
