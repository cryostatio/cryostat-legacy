...
class CredentialsPostHandler extends AbstractV2RequestHandler<Boolean> {
    ...
    @Override
    public IntermediateResponse<Boolean> handle(RequestParameters params) throws ApiException {
        String targetId = params.getPathParams().get("targetId");
        String username;
        String password;
        logger.info("formAttributes: " + params.getFormAttributes());
        try {
            username =
        ...
        return new IntermediateResponse<Boolean>().body(success);
    }
}
