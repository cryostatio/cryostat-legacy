...
class AutoRulesIT extends ExternalTargetsTest {
    ...
    void testAddRuleDisabledAndPatchEnable() throws Exception {
        ...
        try {
            ...
            CompletableFuture<Buffer> deleteRuleResponse = new CompletableFuture<>();
            webClient
                    ...
                    .send(
                            ar -> {
                                if (assertRequestStatus(ar, deleteRuleResponse)) {
                                    MatcherAssert.assertThat(
                                            ar.result().statusCode(), Matchers.equalTo(200));
                                    deleteRuleResponse.complete(ar.result().body());
                                }
                            });
            try {
                deleteRuleResponse.get();
            } catch (InterruptedException | ExecutionException e) {
                throw new ITestCleanupFailedException(
                    ...
                    .send(
                            ar -> {
                                if (assertRequestStatus(ar, deleteRecResponse)) {
                                    deleteRecResponse.complete(ar.result().body());
                                }
                            });
            try {
