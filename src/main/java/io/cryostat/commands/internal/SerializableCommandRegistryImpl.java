...
class SerializableCommandRegistryImpl implements SerializableCommandRegistry {
    ...
    SerializableCommandRegistryImpl(Set<Command> allCommands, Logger logger) {
        ...
        for (SerializableCommand command : commands) {
            ...
            if (commandMap.containsKey(commandName)) {
                throw new CommandRegistryImpl.CommandDefinitionException(
    ...
    @Override
    public SerializableCommand.Output<?> execute(String commandName, String[] args) {
        if (!isCommandRegistered(commandName)) {
            return new SerializableCommand.FailureOutput(
                    String.format("Command \"%s\" not recognized", commandName));
        }
        if (!isCommandAvailable(commandName)) {
            return new SerializableCommand.FailureOutput(
                    String.format("Command \"%s\" unavailable", commandName));
        }
        try {
            ...
            return new SerializableCommand.ExceptionOutput(e);
        }
    }
