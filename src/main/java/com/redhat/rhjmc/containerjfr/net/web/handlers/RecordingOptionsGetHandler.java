...
public class RecordingOptionsGetHandler extends AbstractAuthenticatedRequestHandler {
    ...
    void handleAuthenticated(RoutingContext ctx) throws Exception {
        ...
        RecordingOptionsBuilder builder = recordingOptionsBuilderFactory.create(connection.getService());
        IConstrainedMap<String> recordingOptions = builder.build();

        Map<String, String> response = Collections.emptyMap();
        response.put("toDisk", recordingOptions.get("toDisk").toString());
        response.put("maxAge", recordingOptions.get("maxAge").toString());
        response.put("maxSize", recordingOptions.get("maxSize").toString());
