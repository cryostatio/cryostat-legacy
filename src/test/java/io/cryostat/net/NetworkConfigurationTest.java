...
class NetworkConfigurationTest {
    ...
    void shouldReportCommandChannelHost() throws SocketException, UnknownHostException {
        ...
        Mockito.when(env.getEnv(Mockito.eq("CRYOSTAT_WEB_HOST"), Mockito.anyString()))
                .thenReturn("bar");
        Mockito.when(env.getEnv(Mockito.eq("CRYOSTAT_LISTEN_HOST"), Mockito.anyString()))
                .thenReturn("baz");
        MatcherAssert.assertThat(conf.getCommandChannelHost(), Matchers.equalTo("baz"));
        Mockito.verify(resolver).getHostAddress();
        Mockito.verify(env).getEnv("CRYOSTAT_WEB_HOST", "foo");
        Mockito.verify(env).getEnv("CRYOSTAT_LISTEN_HOST", "bar");
    }
    ...
    void shouldReportInternalCommandChannelPort() {
        Mockito.when(env.getEnv(Mockito.eq("CRYOSTAT_LISTEN_PORT"), Mockito.anyString()))
                .thenReturn("9191");
        MatcherAssert.assertThat(conf.getInternalCommandChannelPort(), Matchers.equalTo(9191));
        Mockito.verify(env).getEnv("CRYOSTAT_LISTEN_PORT", "9090");
    }

    @Test
    void shouldReportExternalCommandChannelPort() {
        Mockito.when(env.getEnv(Mockito.eq("CRYOSTAT_EXT_LISTEN_PORT"), Mockito.anyString()))
                .thenReturn("9292");
        Mockito.when(env.getEnv(Mockito.eq("CRYOSTAT_LISTEN_PORT"), Mockito.anyString()))
                .thenReturn("9191");
        MatcherAssert.assertThat(conf.getExternalCommandChannelPort(), Matchers.equalTo(9292));
        Mockito.verify(env).getEnv("CRYOSTAT_EXT_LISTEN_PORT", "9191");
        Mockito.verify(env).getEnv("CRYOSTAT_LISTEN_PORT", "9090");
    }
